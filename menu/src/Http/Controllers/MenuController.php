...
namespace BlackCMS\Menu\Http\Controllers;
...
use BlackCMS\Menu\Forms\MenuForm;
use BlackCMS\Menu\Http\Requests\MenuRequest;
...
use BlackCMS\Menu\Tables\MenuTable;
...
class MenuController extends BaseController
{
    ...
    public function store(MenuRequest $request, BaseHttpResponse $response)
    {
        ...
        $menu->slug = $this->menuRepository->createSlug(
            $request->input("name")
        );
        ...
        event(new CreatedContentEvent(MENU_MODULE_NAME, $request, $menu));
    ...
    protected function saveMenuLocations($menu, Request $request)
    {
        ...
        foreach ($locations as $location) {
            ...
            event(
                new CreatedContentEvent(
                    MENU_LOCATION_MODULE_NAME,
                    $request,
                    $menuLocation
                )
            );
    ...
    public function edit($id, FormBuilder $formBuilder, Request $request)
    {
        ...
        return $formBuilder
            ->create(MenuForm::class, ["model" => $menu])
            ->renderForm();
    }
    ...
    public function update(
        ...
        event(new UpdatedContentEvent(MENU_MODULE_NAME, $request, $menu));

        $this->saveMenuLocations($menu, $request);

        $deletedNodes = ltrim($request->input("deleted_nodes", ""));
        if ($deletedNodes) {
            $deletedNodes = explode(
                " ",
                ltrim($request->input("deleted_nodes", ""))
            );
        ...
        Menu::recursiveSaveMenu(
            json_decode($request->input("menu_nodes"), true),
            $menu->id,
            0
        );
    ...
    public function destroy(Request $request, $id, BaseHttpResponse $response)
    {
        try {
            ...
            event(new DeletedContentEvent(MENU_MODULE_NAME, $request, $menu));
     ...
     * @return BaseHttpResponse
     */
    public function getNode(Request $request, BaseHttpResponse $response)
    {
        $html = "";
        ...
        foreach ($data as $key => $value) {
            ...
            $row->fill($value);
            $row = Menu::getReferenceMenuNode($value, $row);
            $row->id = time() + $key;
            $html .= view(
                "packages/menu::partials.node",
                compact("row")
            )->render();
        }
