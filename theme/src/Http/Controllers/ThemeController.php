...
namespace BlackCMS\Theme\Http\Controllers;
...
use BlackCMS\Theme\Forms\CustomCSSForm;
use BlackCMS\Theme\Forms\CustomHTMLForm;
use BlackCMS\Theme\Forms\CustomJSForm;
use BlackCMS\Theme\Http\Requests\CustomCssRequest;
use BlackCMS\Theme\Http\Requests\CustomHtmlRequest;
use BlackCMS\Theme\Http\Requests\CustomJsRequest;
use BlackCMS\Theme\Services\ThemeService;
use Exception;
use File;
...
class ThemeController extends BaseController
{
    ...
    public function postActivateTheme(
        ...
        return $response->setMessage(
            trans("packages/theme::theme.active_success")
        );
    }
     ...
     * @return BaseHttpResponse
     */
    public function postCustomCss(
        CustomCssRequest $request,
        BaseHttpResponse $response
    ) {
        File::delete(
            theme_path(
                Theme::getThemeName() . "/public/css/style.integration.css"
            )
        );
        ...
        if (empty($css)) {
            ...
            if (!$saved) {
                return $response->setError()->setMessage(
                    trans("packages/theme::theme.folder_is_not_writeable", [
        ...
        return $response->setMessage(
            trans("packages/theme::theme.update_custom_css_success")
        );
    }
    ...
    public function postCustomJs(
        ...
        return $response->setMessage(
            trans("packages/theme::theme.update_custom_js_success")
        );
    }
    ...
    public function postRemoveTheme(
        ...
        if (in_array($theme, scan_folder(theme_path()))) {
            try {
                ...
                return $response->setMessage(
                    trans("packages/theme::theme.remove_theme_success")
                );
    ...
    public function postCustomHtml(
        ...
        setting()
            ->set(
                "custom_header_html",
                BaseHelper::clean($request->input("header_html") ?: "")
            )
            ->set(
                "custom_body_html",
                BaseHelper::clean($request->input("body_html") ?: "")
            )
            ->set(
                "custom_footer_html",
                BaseHelper::clean($request->input("footer_html") ?: "")
            )
            ->save();

        return $response->setMessage(
            trans("packages/theme::theme.update_custom_html_success")
        );
    }
}
