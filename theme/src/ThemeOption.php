...
class ThemeOption
{
    ...
    public function setSection(array $section = []): self
    {
        ...
        if (!empty($this->optName) && is_array($section) && !empty($section)) {
            if (!isset($section["id"]) && !isset($section["title"])) {
                $this->errors[$this->optName]["section"]["missing_title"] =
                    "Unable to create a section due to missing id and title.";
            ...
            $this->errors[$this->optName]["section"]["empty"] =
                "Unable to create a section due an empty section array or the section variable passed was not an array.";
     ...
     * @param array $fields
     */
    public function processFieldsArray(
        string $sectionId = "",
        array $fields = []
    ): void {
    ...
    public function removeSection(string $id = "", bool $fields = false): self
    {
        if (!empty($this->optName) && !empty($id)) {
            if (isset($this->sections[$this->optName][$id])) {
                ...
                if (isset($this->fields[$this->optName]) &&
                    !empty($this->fields[$this->optName]) &&
                    $fields
                ) {
    ...
    public function hideSection(string $id = "", bool $hide = true): void
    {
        ...
        if (!empty($this->optName) &&
            !empty($id) &&
            isset($this->sections[$this->optName][$id])
        ) {
    ...
    public function hideField(string $id = "", bool $hide = true): void
    {
        ...
        if (!empty($this->optName) &&
            !empty($id) &&
            isset($this->fields[$this->optName][$id])
        ) {
            if (!$hide) {
    ...
    public function setArgs(array $args = []): self
    {
        ...
        if (!empty($this->optName) && !empty($args) && is_array($args)) {
            if (isset($this->args[$this->optName]) &&
                isset($this->args[$this->optName]["clearArgs"])
            ) {
                $this->args[$this->optName] = [];
            }

            $this->args[$this->optName] = parse_args(
                $args,
                $this->args[$this->optName]
            );
        }
    ...
    public function setOption(string $key, ?string $value = ""): self
    {
        ...
        Setting::set(
            $this->getOptionKey($key, $this->getCurrentLocaleCode()),
            $value
        );
    ...
    protected function getCurrentLocaleCode(): ?string
    {
        ...
        $currentLocale = is_in_admin()
            ? Language::getCurrentAdminLocaleCode()
            : Language::getCurrentLocaleCode();

        return $currentLocale &&
            $currentLocale != Language::getDefaultLocaleCode()
            ? "-" . $currentLocale
            : null;
    }
    ...
    public function renderField(array $field): ?string
    {
        try {
            if ($this->hasOption($field["attributes"]["name"])) {
                $field["attributes"]["value"] = $this->getOption(
                    $field["attributes"]["name"]
                );
            }

            return call_user_func_array(
                [Form::class, $field["type"]],
                array_values($field["attributes"])
            );
