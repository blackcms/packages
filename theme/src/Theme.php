...
use BlackCMS\Theme\Exceptions\UnknownLayoutFileException;
use BlackCMS\Theme\Exceptions\UnknownPartialFileException;
use BlackCMS\Theme\Exceptions\UnknownThemeException;
use Closure;
use Exception;
use File;
...
class Theme implements ThemeContract
{
    ...
    public function theme(?string $theme = null)
    {
        ...
        if (!$this->exists($theme) && !app()->runningInConsole()) {
            throw new UnknownThemeException(
        ...
        if ($publicThemeName != $currentTheme) {
            $assetPath =
                substr($assetPath, 0, -strlen($currentTheme)) .
                $publicThemeName;
        }

        // Add asset path to asset container.
        $this->asset->addPath(
            $assetPath . "/" . $this->getConfig("containerDir.asset")
        );
    ...
    public function getConfig(?string $key = null)
    {
        ...
        if ($this->theme &&
            !isset($this->themeConfig["themes"][$this->theme])
        ) {
            ...
            if ($this->files->exists($minorConfigPath)) {
                $this->themeConfig["themes"][
                    $this->theme
                ] = $this->files->getRequire($minorConfigPath);
            }
        }
    ...
    protected function evaluateConfig(array $config): array
    {
        ...
        if (isset($minorConfig["events"]["before"])) {
            $minorConfig["events"]["appendBefore"] =
                $minorConfig["events"]["before"];
    ...
    public function getPublicThemeName()
    {
        ...
        $publicThemeName = $this->config->get(
            "packages.theme.general.public_theme_name"
        );
     ...
     * @return Theme
     */
    protected function appendOrPrepend(
        string $region,
        $value,
        string $type = "append"
    ): self {
    ...
    public function bind(string $variable, $callback = null)
    {
        ...
        return Arr::get($this->bindings, $name, function () use (
            &$events,
            &$bindings,
            $name
        ) {
    ...
    {
        $partialDir = $this->getThemeNamespace(
            $this->getConfig("containerDir.partial")
        );
    ...
    public function loadPartial(string $view, string $partialDir, array $args)
    {
        ...
        if (!$this->view->exists($path)) {
            throw new UnknownPartialFileException(
                "Partial view [" . $view . "] not found."
            );
        }
    ...
    protected function handleViewNotFound($path)
    {
        if (app()->isLocal()) {
            $path = str_replace(
                $this->getThemeNamespace(),
                $this->getThemeName(),
                $path
            );
            $file = str_replace("::", "/", str_replace(".", "/", $path));
            dd(
                'This theme has not supported this view, please create file "' .
                    theme_path($file) .
    ...
    public function location(bool $realPath = false): ?string
    {
        if ($this->view->exists($this->content)) {
            return $realPath
                ? $this->view->getFinder()->find($this->content)
                : $this->content;
        }
    ...
    public function __call($method, $parameters = [])
    {
        ...
        if (in_array($callable[0], ["set", "prepend", "append", "has", "get"])
            ...
            $value = lcfirst(
                preg_replace("|^" . $callable[0] . "|", "", $method)
            );
    ...
    {
        return $this->view
            ->make($this->getThemeNamespace("views") . "." . $view)
            ->render();
    }
