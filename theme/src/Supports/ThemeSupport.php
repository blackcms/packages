...
namespace BlackCMS\Theme\Supports;
...
class ThemeSupport
{
    ...
    public static function registerYoutubeShortcode(string $viewPath = null)
    {
        add_shortcode(
            ...
            function ($shortcode) use ($viewPath) {
                ...
                return view(
                    ($viewPath ?: "packages/theme::shortcodes") . ".youtube",
                    compact("url")
                )->render();
            }
        );

        shortcode()->setAdminConfig("youtube-video", function (
            $attributes,
            $content
        ) use ($viewPath) {
            return view(
                ($viewPath ?: "packages/theme::shortcodes") .
                    ".youtube-admin-config",
    ...
    public static function registerGoogleMapsShortcode(string $viewPath = null)
    {
        add_shortcode(
            "google-map",
            __("Google map"),
            __("Add Google map iframe"),
            function ($shortcode) use ($viewPath) {
                return view(
                    ($viewPath ?: "packages/theme::shortcodes") . ".google-map",
                    ["address" => $shortcode->content]
                )->render();
            }
        );

        shortcode()->setAdminConfig("google-map", function (
            $attributes,
            $content
        ) use ($viewPath) {
            return view(
                ($viewPath ?: "packages/theme::shortcodes") .
                    ".google-map-admin-config",
                compact("attributes", "content")
            )->render();
        });
    }
    ...
    public static function getCustomJS(string $location): string
    {
        ...
        if ((!Str::contains($js, "<script") ||
                !Str::contains($js, "</script>")) &&
