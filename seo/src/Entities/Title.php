...
namespace BlackCMS\Seo\Entities;

use BlackCMS\Seo\Contracts\Entities\TitleContract;
use BlackCMS\Seo\Exceptions\InvalidArgumentException;
...
class Title implements TitleContract
{
    ...
    protected function init()
    {
        ...
        $this->setSeparator(
            config("packages.seo.general.title.separator", "-")
        );
        $this->switchPosition(config("packages.seo.general.title.first", true));
        $this->setMax(config("packages.seo.general.title.max", 55));
    }
    ...
    {
        return empty(($separator = $this->getSeparator()))
            ? " "
            : " " . $separator . " ";
    }
