class SeoManagement {
    ...
    handleMetaBox() {
        ...
        if (permalink.length) {
            $(".page-url-seo p").text(
                permalink.prop("href").replace("?preview=true", "")
            );
        }
        ...
        this.$document.on("keyup", "input[name=name]", (event) => {
            SeoManagement.updateSEOTitle($(event.currentTarget).val());
        });
        ...
        this.$document.on("keyup", "textarea[name=description]", (event) => {
            SeoManagement.updateSEODescription($(event.currentTarget).val());
        });
...
$(document).ready(() => {
    new SeoManagement().handleMetaBox();
});
