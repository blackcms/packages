...
namespace BlackCMS\Page\Tables;
...
class PageTable extends TableAbstract
{
        ...
        $data = $this->table
            ...
            ->editColumn("name", function ($item) {
                if (!Auth::user()->hasPermission("posts.edit")) {
                    ...
                    $name = Html::link(
                        route("pages.edit", $item->id),
                        $item->name
                    );
                }

                if (function_exists("theme_option") &&
                    BaseHelper::isHomepage($item->id)
                ) {
                    $name .= Html::tag(
                        "span",
                        " — " . trans("packages/page::pages.front_page"),
                        [
    ...
    public function query()
    {
        $query = $this->repository
            ->getModel()
            ->select(["id", "name", "template", "created_at", "status"]);
