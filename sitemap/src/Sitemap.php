...
class Sitemap
{
    ...
    public function addItem($params = [])
    {
        ...
        if ($this->model->isEscaping()) {
            ...
            if ($translations) {
                foreach ($translations as $k => $translation) {
                    foreach ($translation as $key => $value) {
                        $translations[$k][$key] = htmlentities(
                            $value,
                            ENT_XML1
                        );
                    }
            ...
            if ($videos) {
                foreach ($videos as $k => $video) {
                    if (!empty($video["title"])) {
                        $videos[$k]["title"] = htmlentities(
                            $video["title"],
                            ENT_XML1
                        );
                    }
                    if (!empty($video["description"])) {
                        $videos[$k]["description"] = htmlentities(
                            $video["description"],
                            ENT_XML1
                        );
                    }
            ...
            if ($googlenews) {
                if (isset($googlenews["sitename"])) {
                    $googlenews["sitename"] = htmlentities(
                        $googlenews["sitename"],
                        ENT_XML1
                    );
                }
        ...
        $googlenews["sitename"] = isset($googlenews["sitename"])
            ? $googlenews["sitename"]
            : "";
        $googlenews["language"] = isset($googlenews["language"])
            ? $googlenews["language"]
            : "en";
        $googlenews["publication_date"] = isset($googlenews["publication_date"])
            ? $googlenews["publication_date"]
            : date("Y-m-d H:i:s");
    ...
    public function render($format = "xml")
    {
        ...
        if ($this->model->getMaxSize() > 0 &&
            count($this->model->getItems()) > $this->model->getMaxSize()
        ) {
            $this->model->limitSize($this->model->getMaxSize());
        } elseif ("google-news" == $format &&
            count($this->model->getItems()) > 1000
        ) {
            $this->model->limitSize(1000);
        } elseif ("google-news" != $format &&
            count($this->model->getItems()) > 50000
        ) {
    ...
    {
        return $this->model->isUseCache() &&
            $this->cache->has($this->model->getCacheKey());
    }
